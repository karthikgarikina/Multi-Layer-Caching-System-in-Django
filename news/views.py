...
def maybe_cache_page(view):
    return cache_page(60 * 15)(view) if settings.CACHE_ENABLED else view
...
def article_detail(request, pk):
    ...
    response["X-Fragment-Cache-Key"] = f"article_body:{article.pk}:{article.updated_at.timestamp()}"
    return response
